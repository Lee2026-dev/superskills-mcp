...
export function assertSafeSkill(skill: ResolvedSkill): void {
  // 1. skillDir 必须存在且是目录
  ...
  for (const arg of skill.runner.args) {
    ...
    if (!inServerDir && !inSkillDir) {
      throw new Error(
        `[${skill.name}] runner script must be inside serverDir or skillDir: ${arg}`
      );
    }
  }
}

export function assertSafeSkills(skills: ResolvedSkill[]): void {
