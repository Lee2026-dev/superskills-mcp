...
function resolveRunnerArgs(args) {
...
export function loadConfig() {
    ...
    const defaults = {
        ...
        runner: resolveRunner(raw.defaults?.runner ?? { command: "bun", args: ["{serverDir}/scripts/mcp-adapter.ts"] })
    };
    ...
    const skills = raw.skills.map((s, i) => {
        if (!s.name)
            throw new Error(`skills[${i}] is missing 'name'`);
        if (!s.skillDir)
            throw new Error(`skills[${i}] ('${s.name}') is missing 'skillDir'`);
        return {
            name: s.name,
