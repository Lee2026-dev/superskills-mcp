...
export async function runSkill(skill, args) {
    ...
    if (exit.code !== 0) {
        throw new SkillRunnerError(`Skill process failed with code=${exit.code}, signal=${exit.signal}. stderr=${stderrText}`);
    }
    ...
    if (parsed.ok === false) {
        throw new SkillRunnerError(parsed.error?.message ?? parsed.error?.code ?? "Skill returned ok=false");
    }
